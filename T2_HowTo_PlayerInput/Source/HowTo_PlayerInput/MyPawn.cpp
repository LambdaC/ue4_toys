...
void AMyPawn::Tick(float DeltaTime)
{
    ...
    float CurrentSacle = OurVisibleComponent->GetComponentScale().X;
    if (bGrowing)
    {
        // Grow to double size over the course of one second
        CurrentSacle += DeltaTime;
    }
    else
    {
        // Shrink half as fast as we grow
        CurrentSacle -= (DeltaTime * 0.5f);
    }

    // Make sure we never drop below our starting size, or increase past double size.
    CurrentSacle = FMath::Clamp(CurrentSacle, 1.0f, 2.0f);
    OurVisibleComponent->SetWorldScale3D(FVector(CurrentSacle));
